...
export function getCompletionScriptPath(shell) {
    ...
    const scriptName = scriptNames[shell];
    if (!scriptName) return undefined;

    const candidatePaths = [
        path.join(__dirname, "scripts", scriptName),
        path.resolve(__dirname, "../../lib/completions/scripts", scriptName)
    ];

    return candidatePaths.find((scriptPath) => fs.existsSync(scriptPath)) || candidatePaths[0];
}
