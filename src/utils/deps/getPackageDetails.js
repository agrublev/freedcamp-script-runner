...
function getSizeAndLicenseFromGitHub(owner, repo) {
    ...
    return fetch(repoInfoUrl)
        .then(checkResponse)
        .then(({ size: sizeKb, license: licenseObj, updated_at: modified }) => {
 ...
 * @return {Promise}
 */
module.exports = function getPackageDetails(name, versionLoose) {
    ...
    if (versionUrlObj.protocol) {
        ...
        printError(
            `${versionUrlObj.protocol} is not supported by npm-consider, skipping ${versionLoose}`
        );
        return Promise.resolve(null);
    }
    ...
    if (!packageDetailsCache[key]) {
        ...
        packageDetailsCache[key] = fetch(infoUrl)
            .then(checkResponse)
            .then(packageInfo => {
                let version;
                if (!versionLoose) {
