...
function calculateNewLicenses(impactLicenses, currentLicenses) {
    return Object.keys(impactLicenses)
        .filter(newLicense => {
            return !Object.keys(currentLicenses).some(existingLicense => {
                return satisfies(newLicense, existingLicense);
            });
        })
        .reduce((newLicenses, license, k, licenses) => {
            const matchingL = licenses
                .filter(otherLicense => {
...
module.exports = function showImpact(name, versionLoose, newPackages) {
    return getLocalPackage().then(localPackage => {
        ...
        return walkDependencies(allDependencies).then(currentPackages => {
            store.set("deps." + name, currentPackages);
            ...
            if (Object.keys(newLicenses).length) {
                Object.keys(newLicenses).forEach((license, k) => {
                    table.push([k === 0 ? "Licenses" : "", license, newLicenses[license]]);
                });
            } else {
                table.push([
                    {
