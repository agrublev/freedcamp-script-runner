...
const otp = process.argv.slice(2)[1];
var client = github({
    version: 3,
    auth: { username: "agrublev", password: pass, otp: otp }
});
...
const release = async () => {
    try {
        ...
        await client.pull(
            {
                user: "agrublev",
                repo: "freedcamp-script-runner",
                branch: "MyName" // statusSummary.current
            },
            {
                user: "agrublev",
                repo: "freedcamp-script-runner",
