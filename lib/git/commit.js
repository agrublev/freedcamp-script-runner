...
function getAnthropicApiKey() {
    ...
    throw new Error(
        "ANTHROPIC_API_KEY is not exported. Add `export ANTHROPIC_API_KEY=\"your_key\"` to ~/.zshrc or ~/.bashrc, then restart your shell."
    );
}
...
const commit = async () => {
    ...
    // Only files the user explicitly consented to commit. Used to strip any
    // untracked file Claude hallucinates into a proposal that was not selected.
    ...
    // Filter each proposed commit to only the files the user consented to.
    // Drop commits that become empty after filtering — Claude sometimes
    // references untracked files the user did not select.
    const commits = result.commits
        .map(c => ({ ...c, files: (c.files || []).filter(f => allowedFiles.has(f)) }))
        .filter(c => c.files.length > 0);

    if (commits.length === 0) {
        fsrLog.error("No commits remain after filtering to selected files.");
        return;
    }
