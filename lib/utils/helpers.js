...
utils.emptyDir = async (f) => {
    try {
        await fs.emptyDir(f);
        // fsrLog.log(`${chalk.green.underline("Directory")} ${chalk.bold(f)} emptied!`);
    } catch (err) {
        fsrLog.error(err);
    }
};
...
utils.readJson = async (f) => {
    try {
        ...
        // fsrLog.log(`${chalk.green.underline("File")} ${chalk.bold(f)} read!`);
        return packageObj;
    } catch (err) {
        fsrLog.error(err);
        return {};
...
utils.readFile = async (f) => {
    try {
        return await fs.readFileSync(f, "utf8");
    } catch (err) {
        fsrLog.error(err);
        return null;
    }
...
utils.removeFile = async (f) => {
    try {
        // fsrLog.log(`${chalk.green.underline("File")} ${chalk.bold(f)} removed!`);
        return await fs.remove(f);
    } catch (err) {
        fsrLog.error(`File ${f} NOT REMOVED! ${err}`);
        return false;
    }
};
...
utils.writeFile = async (f, contents = "") => {
    try {
        ...
        // fsrLog.log(`${chalk.green.underline("File")} ${chalk.bold(f)} written!`);
    } catch (err) {
        fsrLog.error(err);
    }
};
