...
class CacheMonitor {
    ...
    printStats(options = {}) {
        ...
        fsrLog.log(`  Size: ${chalk.green(stats.cache.size)}/${stats.cache.maxSize}`);
        fsrLog.log(`  Hits: ${chalk.green(stats.cache.hits)}`);
        fsrLog.log(`  Misses: ${chalk.yellow(stats.cache.misses)}`);
        fsrLog.log(`  Hit Rate: ${this._colorizeHitRate(stats.cache.hitRate)}%`);
        fsrLog.log(`  Invalidations: ${chalk.yellow(stats.cache.invalidations)}`);
        fsrLog.log(`  Evictions: ${chalk.yellow(stats.cache.evictions)}`);

        // Performance stats
        if (stats.performance.warmCacheSamples > 0) {
            fsrLog.log(chalk.bold('\nPerformance:'));
            fsrLog.log(`  Cold Cache: ${chalk.yellow(stats.performance.avgColdTime + 'ms')}`);
            fsrLog.log(`  Warm Cache: ${chalk.green(stats.performance.avgWarmTime + 'ms')}`);
            fsrLog.log(`  Speedup: ${chalk.bold.green(stats.performance.speedup)}`);
        }

        // File watcher stats
        if (stats.watcher) {
            fsrLog.log(chalk.bold('\nFile Watcher:'));
            fsrLog.log(`  Files Watched: ${chalk.green(stats.watcher.filesWatched)}`);
            fsrLog.log(`  Change Events: ${chalk.yellow(stats.watcher.changeEvents)}`);
            fsrLog.log(`  Invalidations: ${chalk.yellow(stats.watcher.invalidations)}`);

            if (verbose && stats.watcher.watchedFiles.length > 0) {
                fsrLog.log(chalk.bold('\n  Watched Files:'));
                stats.watcher.watchedFiles.forEach(file => {
                    fsrLog.log(`    - ${chalk.gray(file)}`);
                });
            }
        }
