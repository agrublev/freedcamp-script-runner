...
function normalizeEntry(entry) {
    ...
    return {
        ...
        changesLog: Array.isArray(entry.changesLog) ? entry.changesLog.filter((v) => typeof v === "string") : []
    };
}
...
function normalizeCache(raw) {
    ...
    const entriesRaw = raw.entries && typeof raw.entries === "object" && !Array.isArray(raw.entries)
        ? raw.entries
        : {};
...
export async function writeCacheEntry(key, value) {
    assertCacheKey(key);
