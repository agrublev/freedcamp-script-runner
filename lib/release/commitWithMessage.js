        ...
        .then(({ commitTitle }) => {
            ...
            inquirer
                ...
                .then(async ({ commitDescription }) => {
                    ...
                    try {
                        // logInfo(command);
                        await childProcess.execSync("git -c credential.helper= -c core.quotepath=false -c log.showSignature=false push --progress --porcelain origin refs/heads/TT:TT --set-upstream --tags", {
                            stdio: "inherit",
