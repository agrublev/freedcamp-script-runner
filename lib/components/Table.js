...
const Cell = ({ children }) => <Color>{children}</Color>;
...
const intersperse = val => vals =>
    vals.reduce((s, c, i) => (isEmpty(s) ? [c] : [...s, val(i), c]), []);
const fillWith = el => length => str => `${str}${el.repeat(length - str.length)}`;
const getCells = columns => data =>
    columns.map(({ width, key }) => ({ width, key, value: get(key)(data) }));
...
const line = (key, Cell, Skeleton, { line, left, right, cross, padding }) => (
    cells,
    index = ""
) => {
    ...
    const columns = cells.map(({ width, key, value }, i) => (
        <Cell key={key + String(i)} cellData={{ key, value, row: index, column: i, width }}>
            {line.repeat(padding)}
