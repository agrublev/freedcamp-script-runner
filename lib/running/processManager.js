 ...
 *   1. options.prefixColors[index]  — per-task CLI override array
 *   2. taskData.color               — color: tag in fscripts.md description
 *   3. PREFIX_COLORS cycle          — automatic fallback
 */
function buildProcess(taskName, taskData, index, options = {}) {
    ...
    const prefixColor =
        (options.prefixColors && options.prefixColors[index]) ||
        taskData.color ||
        PREFIX_COLORS[index % PREFIX_COLORS.length];
