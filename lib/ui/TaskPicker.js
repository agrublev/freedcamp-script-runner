        ...
        for (const [name, task] of Object.entries(rootCat.tasks)) {
            list.push({
                ...
                sublabel: task.description
                    ? task.description.replace(/\n/g, " ").trim().slice(0, maxSub - name.trim().length)
                    : "",
                value: name.trim(),
                task
            });
...
export default function TaskPicker({ FcScripts, recentTaskOptions, onSelect }) {
    ...
    useInput((input, key) => {
        if (key.tab) { setExpanded(e => !e); return; }
        if (key.upArrow) { setExpanded(false); setIdx(c => nextSel(list, c, -1)); }
        else if (key.downArrow) { setExpanded(false); setIdx(c => nextSel(list, c, 1)); }
        else if (key.return) {
            const item = list[idx];
            if (!item?.sel) return;
            if (item._t === 'cat') {
            ...
            <Box flexGrow={1} height={listHeight}>
                <ScrollList ref={listRef} selectedIndex={idx} height={listHeight}>
                    {list.map((item, i) => {
                        ...
                        const fullDesc = isSel && expanded && item._t === 'task'
                            ? ((item.task?.description) || (activeCat?.tasks?.[item.value]?.description) || '')
                            : null;
                        return (
                            <Box key={item.value || i} flexDirection="column">
                                <Box gap={1}>
                                    ...
                                    {!expanded && item.sublabel && <Text dimColor color="gray">~  {item.sublabel}</Text>}
                                </Box>
                                {isSel && expanded && fullDesc
                                    ? <Box marginLeft={2} marginBottom={1}><Text color="gray" wrap="wrap">{fullDesc.trim()}</Text></Box>
                                    : null
                                }
                            </Box>
                        );
