    ...
    for (const check of checks) {
        try {
            ...
            if (fix && !result.passed && result.canFix && result.fix) {
                try {
                    fsrLog.log(chalk.yellow(`   Attempting to fix...`));
                    const fixMessage = await result.fix();
                    fsrLog.log(chalk.green(`   ✓ Fixed: ${fixMessage}\n`));
                    fixedCount++;
                } catch (fixError) {
                    fsrLog.log(chalk.red(`   ✗ Fix failed: ${fixError.message}\n`));
                }
            } else if (!result.passed && result.fixInstructions) {
                fsrLog.log(chalk.gray(`   ${result.fixInstructions}\n`));
            }
        } catch (error) {
            fsrLog.log(
...
function displayCheckResult(result) {
    ...
    fsrLog.log(chalk[color](`${icon} ${result.name}`));
    fsrLog.log(chalk.gray(`   ${result.message}\n`));
}
...
function displaySummary(passed, warnings, errors, fixed) {
    fsrLog.log(chalk.gray('─'.repeat(50)));
    fsrLog.log(chalk.bold('Summary:'));

    if (passed > 0) {
        fsrLog.log(chalk.green(`  ✅ Passed: ${passed}`));
    }

    if (warnings > 0) {
        fsrLog.log(chalk.yellow(`  ⚠️  Warnings: ${warnings}`));
    }

    if (errors > 0) {
        fsrLog.log(chalk.red(`  ❌ Errors: ${errors}`));
    }

    if (fixed > 0) {
        fsrLog.log(chalk.cyan(`  🔧 Fixed: ${fixed}`));
    }
...
export default async function doctor(argv) {
    try {
        ...
        if (options.json) {
            fsrLog.log(JSON.stringify(result, null, 2));
        }
        ...
        fsrLog.error(chalk.red.bold(`\n❌ Doctor command failed: ${error.message}\n`));
        if (argv.verbose) {
            fsrLog.error(error.stack);
        }
        process.exit(1);
    }
}
